...
import { unbindStatistical } from "@/api/recommend";
// 引入公共方法
...
export default {
              ...
              return h("div", [
                h(
                  "el-button",
                  {
                    props: { type: "primary" },
                    style: {
                      display: params.row.is_Unbinding ? "unset" : "none",
                    },
                    on: {
                      click: () => {
  ...
  methods: {
    ...
    // 解绑操作
    async unbind(id) {
      this.$confirm("确认解绑当前新注册用户吗?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(async () => {
          let res = await unbindStatistical({ id });
          if (res.code === 2000) {
            this.$success("解绑成功!");
            this.getList();
          }
