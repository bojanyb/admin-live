...
<script>
// 引入api
import { getPromoterSub, deleteParams, setBindStatus } from "@/api/recommend";
// 进入新增推广商组件
...
export default {
      ...
      return {
        ...
        columns: [
          ...
          },
          {
            ...
            render: (h, params) => {
              let arr = ["推广商", "推广组", "推广员"];
              let data = arr.find((a, b) => {
                return b + 1 === params.row.level;
              });
              return h(
                "span",
                data + "ID" + " - " + params.row.user_number || "无"
              );
            },
          },
          {
              ...
              return h("div", [
                h(
                  "el-button",
                  {
                    ...
                    on: {
                      click: () => {
                        this.update(params.row, params.row.level);
                      },
                    },
                  },
                  "修改"
                ...
                h(
                  "el-button",
                  {
                    ...
                    on: {
                      click: () => {
                        this.add(params.row.level + 1, params.row);
                      },
                    },
                  },
                  "新增"
  ...
  methods: {
    // 懒加载
    async loadLazy(tree, treeNode, resolve, callback) {
      // 当前废弃 - 以后需要做懒加载时使用
    ...
    refresh() {
      ...
      this.$set(
        this.$refs.tableList.$refs.table.store.states.lazyTreeNodeMap,
        id,
        []
      ); // 更新节点的值
    ...
    async handleBind(row) {
      let res = await setBindStatus({
        id: row.id,
        status: row.bind_status === 2 ? 1 : row.bind_status === 1 ? 2 : 0,
      });
      if (+res.code === 2000) {
        this.$success("操作成功");
        this.getList();
