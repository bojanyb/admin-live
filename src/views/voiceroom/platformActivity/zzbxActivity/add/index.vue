            ...
            <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="108px" class="demo-ruleForm">
                <el-form-item label="活动名称" prop="activity_type_id">
                    <el-select v-model="ruleForm.activity_type_id" :disabled="disabeld" placeholder="请选择活动名称">
                        <el-option v-for="item in activetyList" :key="item.id" :label="item.name" :value="item.id"></el-option>
                    </el-select>
                </el-form-item>
        ...
        EndPicker() {
            return {
                disabledDate: (time) => {
                    ...
                    return time.getTime() < this.ruleForm.start_time;
                }
            }
        },
        disabeld() {
                ...
                if (valid) {
                    ...
                    s.gifts.forEach(item => {
                        params.gifts.push({
                            id: item.id,
                            type: item.type,
                            probability: item.probability * 100000
                        })
                    })
