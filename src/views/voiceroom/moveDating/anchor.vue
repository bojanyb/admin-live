...
<script>
// 引入api
import { rmHeartAnchorV2, serachTagV2 } from "@/api/moveDating";
// 引入新增组件
...
export default {
  ...
  computed: {
    ...
    cfgs() {
      return {
        vm: this,
        url: REQUEST.move.heartAnchorV2,
        columns: [
          {
            label: "排名",
            prop: "",
          ...
          },
          {
            label: "音色",
            render: (h, params) => {
              let data = this.sound_tagList.find((item) => {
                return item.id === params.row.sound_type;
              });
              return h("span", data ? data.sound_tag : "无");
            },
          },
          {
            label: "性别",
              ...
              return h("div", [
                ...
                h(
                  "el-button",
                  {
                    ...
                    on: {
                      click: () => {
                        this.deleteParams(params.row.anchor_id);
                      },
                    },
                  },
                  "移除"
  ...
  methods: {
    ...
    async deleteParams(anchor_id) {
      this.$confirm("确认移除当前心动主播?", "提示", {
        ...
        .then(async () => {
          let res = await rmHeartAnchorV2({ anchor_id });
          if (res.code === 2000) {
            this.$success("删除成功");
            this.getList();
          }
    ...
    async serachTagFunc() {
      ...
      this.sound_tagList = [{ sound_tag: "全部", id: 0 }, ...list];
    },
  },
  created() {
