    ...
    <el-dialog :title="popTitle" :visible.sync="popFormVisible">
      <el-form :model="popForm">
        <el-form-item label="序号" :label-width="formLabelWidth">
          <el-col :span="7">
            <el-input v-model="popForm.sort" style="width: 235px;" clearable autocomplete="off" />
          </el-col>
        </el-form-item>
...
export default {
  ...
  methods: {
        ...
        if (response && response.data.list.length > 0) {
          ...
          this.list.map(function(item) {
            item.create_timeText = moment(item.create_time * 1000).format(
              'YYYY-MM-DD HH:MM:SS')
            item.start_timeText = moment(item.start_time * 1000).format(
              'YYYY-MM-DD HH:MM:SS')
    ...
    handleEdit(row) {
      ...
      this.popForm.start_time = row.start_time
