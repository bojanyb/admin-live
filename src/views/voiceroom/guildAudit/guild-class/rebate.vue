...
// 引入新增 - 修改组件
import rebateComp from "./components/rebateComp.vue";
// 引入菜单组件
...
export default {
  ...
  computed: {
    ...
    cfgs() {
      return {
        vm: this,
        url: REQUEST.guild.getGuildRebateList,
        columns: [
          {
            label: "公会类型",
            render: (h, params) => {
              ...
              return h("div", [
                ...
                h(
                  "el-button",
                  {
                    ...
                    on: {
                      click: () => {
                        this.deleteParams(params.row.id, 1);
                      },
                    },
                  },
                  "删除"
  ...
  methods: {
    ...
    load(status, row) {
      ...
      setTimeout(() => {
        this.$refs.rebateComp.loadParams(status, row);
      }, 50);
    },
    // 销毁组件
    destoryComp() {
