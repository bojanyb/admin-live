<template>
  <div class="systemConfig-guild-class-box">
    ...
    <!-- 新增 - 修改组件 -->
    <effectiveComp v-if="isDestoryComp" ref="effectiveComp" @destoryComp="destoryComp" @getList="getList"></effectiveComp>
  </div>
</template>
...
export default {
      ...
      return {
        ...
        columns: [
          ...
          },
          {
            label: '有效人数',
            render: (h, params) => {
              return h('span', params.row.eff_user_total + '人')
            }
          },
          {
            label: '奖励类型',
  ...
  methods: {
    ...
    load(status, row) {
      ...
      setTimeout(() => {
        this.$refs.effectiveComp.loadParams(status, row)
      }, 50);
    },
    // 销毁组件
    destoryComp() {
