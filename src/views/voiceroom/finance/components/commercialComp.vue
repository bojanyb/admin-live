<template>
  <div class="finance-commercialComp-box">
    <el-dialog :title="title" :visible.sync="dialogVisible" width="610px" :before-close="handleClose" @closed="closed">
...
export default {
  ...
  methods: {
    // 文件上传
    async upLoad(file) {
      // 商户平台是微信，上传文件则需要多传递字段: merchantId
      let merchantId = ""
      if (this.ruleForm.channel == 2) {
        if (this.ruleForm.config_json == "") {
          this.$message.error("请先填写配置信息")
          this.fileList = [];
          return
        } else {
          let config_json = JSON.parse(this.ruleForm.config_json)
      ...
      // 商户平台为微信环境下 上传新增字段 merchantId
      if (merchantId !== "" && this.ruleForm.channel == 2) {
