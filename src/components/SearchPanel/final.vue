  ...
  <div class="share-filter-grid-box">
    ...
    <div class="container">
      <div class="center">
        ...
        >
          <el-form-item
            v-for="(item, index) in finalForms"
            :key="index"
            :label="item.label"
            :prop="item.name"
          >
            ...
            >
              ...
              <el-button
                ...
                icon="el-icon-search"
                @click="
                  item.handler.onSearch(form[item.name], form[item.selectName])
                "
              />
            </el-input>
            <!--datePicker -->
            <template v-else-if="item.type === 'datePicker'">
              <div class="datePicker">
                ...
                <el-date-picker
                  ...
                  :picker-options="
                    item.pickerOptions || dateParams.pickerOptions
                  "
                  range-separator="-"
          ...
          <el-form-item>
            <el-button
              ...
              >查询</el-button
            >
            <el-button v-if="showAdd" type="success" @click="add">{{
              addName || "新增"
            }}</el-button>
            <el-button v-if="showSave" type="success" @click="onSave"
              >保存</el-button
            >
            <el-button v-if="showToday" @click="today">今天</el-button>
            <el-button v-if="showYesterday" @click="yesterday">昨天</el-button>
            <el-button v-if="showBeforeYesterday" @click="beforeYesterday"
              >前天</el-button
            >
            <el-button v-if="showRecentSeven" @click="recentSeven"
              >七天</el-button
            >
            <el-button v-if="showBigBeforeYesterday" @click="bigBeforeYesterday"
              >大前天</el-button
            >
            <el-button v-if="showCurrentWeek" @click="currentWeek"
              >本周</el-button
            >
            <el-button v-if="showLastWeek" @click="lastWeek">上周</el-button>
            <el-button v-if="showCurrentMonth" @click="currentMonth"
              >本月</el-button
            >
            <el-button v-if="showLastMonth" @click="lastMonth">上月</el-button>
            <el-button v-if="showReset" icon="el-icon-refresh" @click="reset"
              >重置</el-button
            >
            <el-button v-if="showBatchPass" type="success" @click="batchPass">{{
              batchFuncName || "批量通过"
            }}</el-button>
            <el-button v-if="showBatchRurn" type="danger" @click="BatchRurn">{{
              batchRurnName || "批量拒绝"
            }}</el-button>
            <el-button
              v-if="showExport"
              ...
              @click="handleExport"
              >{{ exportName || "导出Excel" }}</el-button
            >
            <el-button v-if="showCustom" type="warning" @click="handleCustom">{{
              customName || "自定义"
            }}</el-button>
            <el-button v-if="showQuery" type="primary" @click="handleQuery">{{
              queryName || "查询"
            }}</el-button>
            <el-button
              v-if="showCurrentPeriodOrder"
              type="warning"
              @click="handleCurrentPeriodOrder"
              >{{ currentPeriodOrderName || "当前时段补单" }}</el-button
            >
            <el-button
              v-if="showCurrentPeriodOrderRes"
              type="primary"
              @click="handleCurrentPeriodOrderRes"
              >{{ currentPeriodOrderResName || "当前时段补单" }}</el-button
            >
          </el-form-item>
        </el-form>
      </div>
        ...
        if (n.length > 0 && n !== o) {
          ...
          for (const i in n) {
            ...
            if (form.type === "datePicker") {
              ...
              if (dateOptis.length) {
                dateType = dateOptis[0].dateType
                  ? dateOptis[0].dateType
                  : "datetimerange";
                pickerOptions = dateOptis[0].pickerOptions
                  ? dateOptis[0].pickerOptions
                  : null;
              } else {
  ...
  methods: {
    ...
    onSelectStock(argument, item) {
      if (argument && argument.length) {
        ...
        item.handler &&
          item.handler.onSelectStock &&
          item.handler.onSelectStock(data);
