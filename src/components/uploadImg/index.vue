<template>
  <div class="uploadImg">
    ...
    >
      ...
      <svgComp
        v-if="url && isSpecial === 'svga' && isShowSvg"
        ref="svgComp"
        :src="url"
        :styleObj="{ width: '178px', height: '178px' }"
      ></svgComp>
      <i v-if="!url" class="el-icon-plus avatar-uploader-icon"></i>
      <span class="fileName" v-if="url && isSpecial === 'zip'"
        >已上传zip文件</span
      >
      <videoPlayerComp
        v-if="url && isSpecial === 'mp4'"
        ref="videoPlayerComp"
        :url="url"
...
export default {
  ...
  computed: {
    ...
    isSpecial() {
      ...
      if (this.url) {
        if (
          this.url.indexOf(".png") !== -1 ||
          this.url.indexOf(".webp") !== -1 ||
          this.url.indexOf(".gif") !== -1 ||
          this.url.indexOf(".jpg") !== -1 ||
          this.url.indexOf(".jpeg") !== -1
        ) {
          return "png";
        } else if (this.url.indexOf(".svga") !== -1) {
          return "svga";
        } else if (
          this.url.indexOf(".zip") !== -1 ||
          this.url.indexOf(".mp3") !== -1 ||
          this.url.indexOf(".m4a") !== -1
        ) {
          return "zip";
