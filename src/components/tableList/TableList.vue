<template>
  <article class="share-table-list-box">
    <el-table
      :data="data"
      style="width: 100%"
      :size="cfgs.size ? cfgs.size : 'medium'"
      :stripe="cfgs.stripe"
    ...
    >
      ...
      <el-table-column
        ...
        width="55"
        align="center"
        v-if="cfgs.isShowCheckbox"
      ></el-table-column>
      <!-- 表格序号 -->
      <el-table-column
        type="index"
        v-if="cfgs.isShowIndex"
        label="序号"
        width="80"
        align="center"
      ></el-table-column>
      ...
      <el-table-column
        ...
        v-if="heckoutGoodsField(item.label)"
      >
        ...
        <template slot-scope="scope">
          ...
          <imgListComp
            ...
            :srcList="
              returnImg(
                scope.row[item.prop] && scope.row[item.prop].length
                  ? scope.row[item.prop]
                  : scope.row[item.propCopy]
              )
            "
          ...
          <div v-if="item.isImgOrText">
            ...
            <!-- 文字 -->
            <div v-else-if="scope.row.msg_type == 1 || !scope.row.msg_type">
              {{ scope.row.content }}
            </div>
          </div>
          <el-switch
    ...
    <div class="pagination" v-if="data.length != 0 && !isHidePage">
      <el-pagination
        ...
        :layout="layout"
        :page-sizes="cfgs.search ? cfgs.search.sizes : search.sizes"
        :total="search.total"
        class="fr"
        :small="isSmall"
      >
...
export default {
  ...
  methods: {
    ...
    handlePageChange(val) {
      this.$emit("handleSizeChange", val);
      if (
        window.location.href.indexOf("/serviceConfig/message-history") !== -1
      ) {
        // 如果页面是消息记录 - 需要去当前页处理
    ...
    getData(max_id, tabIndex) {
      ...
      if (typeof vm.beforeSearch === "function") {
        ...
        if (
          window.location.href.indexOf("/serviceConfig/message-history") !==
            -1 &&
          tabIndex !== "2"
        ) {
          // 模拟缓存当前列表数据 - 仅限消息记录页面使用
